...
export const BssOperationsPage = () => {
    ...
    return <Box><PageHeader title="Business Services" subtitle="Authoritative customers, service catalog, and customer service lifecycle" breadcrumbs={[{ label: 'BSS' }]} actions={tab === 0 ? <Can permission="bss.customers.create"><Button variant="contained" startIcon={<AddIcon />} onClick={() => setCustomerForm({} as Customer)}>Add customer</Button></Can> : <Can permission="bss.services.create"><Button variant="contained" startIcon={<AddIcon />} onClick={() => setServiceForm({} as Service)}>Add service</Button></Can>} />
        ...
        <Stack spacing={1}>{(data.data?.data || []).map((record: Customer | Service) => <Box key={record.id} sx={{ p: 2, border: '1px solid', borderColor: 'divider', borderRadius: 1, display: 'flex', justifyContent: 'space-between', gap: 2, alignItems: 'center' }}><Box onClick={() => navigate('customer_code' in record ? `/bss/customers/${record.id}` : `/bss/services/${record.id}`)} sx={{ cursor: 'pointer' }}><Typography fontWeight={700}>{'customer_code' in record ? record.customer_code : record.service_code} · {record.name}</Typography><Typography variant="body2" color="text.secondary">Company #{record.company_id} · {record.type}{'email' in record && record.email ? ` · ${record.email}` : ''}</Typography></Box><Stack direction="row" spacing={1} alignItems="center"><Chip size="small" label={record.status} color={statusColor(record.status)} />{'customer_code' in record && <Can permission="bss.customers.retire"><Button color="error" size="small" onClick={() => retireCustomer.mutate(record.id)}>Retire</Button></Can>}</Stack></Box>)}</Stack>
