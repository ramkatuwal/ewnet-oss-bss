...
export const CompaniesPage: React.FC = () => {
    ...
    if (!canView) {
        return (
            <Box p={3}>
                <Alert severity="warning">You do not have permission to view companies.</Alert>
            </Box>
        );
    }
    ...
    return (
        <Box>
            <Box display="flex" justifyContent="space-between" alignItems="center" mb={2}>
                ...
                {canCreate && (
                    <Button variant="contained" startIcon={<Add />} onClick={() => { setEditingCompany(null); setFormData({ country: 'Nepal', is_active: true }); setDialogOpen(true); }}>
                        Add Company
                    </Button>
                )}
            </Box>
                            ...
                            {companies.length === 0 ? (
                                ...
                                companies.map((company) => (
                                    <TableRow key={company.id}>
                                        ...
                                        <TableCell>
                                            <Chip label={company.is_active ? 'Active' : 'Inactive'} color={company.is_active ? 'success' : 'default'} size="small" />
                                        </TableCell>
                                        <TableCell align="right">
                                            {canUpdate && (
                                                <IconButton size="small" onClick={() => { setEditingCompany(company); setFormData(company); setDialogOpen(true); }}>
                                                    <Edit fontSize="small" />
                                                </IconButton>
                                            )}
                                            {canDelete && (
            ...
            <Dialog open={dialogOpen} onClose={() => setDialogOpen(false)} maxWidth="md" fullWidth>
                ...
                <DialogActions>
                    ...
                    <Button variant="contained" onClick={handleSubmit} disabled={submitting}>
                        {submitting ? <CircularProgress size={24} /> : (editingCompany ? 'Update' : 'Create')}
                    </Button>
                </DialogActions>
            </Dialog>
        </Box>
