...
class LibreNmsObjectTest extends TestCase
{
    ...
    public function test_upsert_creates_new_object(): void
    {
        ...
        $obj = LibreNmsObject::where('integration_id', $this->integration->id)
            ->where('object_type', 'device')
            ->where('external_id', '87')
            ->first();
        $this->assertNotNull($obj);
        $this->assertSame('router1', $obj->display_name);
    }
    ...
    public function test_upsert_updates_changed_object(): void
    {
        ...
        $count = LibreNmsObject::where('integration_id', $this->integration->id)->count();
